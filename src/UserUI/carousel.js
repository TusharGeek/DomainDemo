...
function Carousel(props){
    ...
    const [Scroll,setScroll] = useState(Defaultstyles);
    ...
    return (
        <div>
        <h1 className="CarouselHeading">{props.CarouselHeading}</h1>
 <div className="carousel">
  
  <button className="handle left-Handle" onClick={handleLeftClick}><ArrowBackIosIcon fontSize="large" /></button>
   <div className="Slider" style ={Scroll}>
   {
       Movies.map((e)=>{
           return <div className="Items"><img src={e.src}></img></div>
       })
   }
   </div>
   <button className="handle right-Handle" onClick={handleRightClick}><ArrowForwardIosIcon fontSize="large" /></button>
  </div>
        </div>
